...
class KMapNet :  public Object, ISerializable{
  /**
	 *
	 */
...
void serialize(std::ostream& out) const override {
   ...
   */
  KMapNet(string name) : ISerializable(){
    this->setName(name);
   ...
   */
  void show_array(string name, double* arr) {
    if (textArea1 != NULL) {
      textArea1.append("\n" + name + "= ");
  ...
  void readInputs() {
    ...
    int inx = 0;
    double* tempRec = (double[]) data[recInx];  // get record
  ...
  void computeDistances() {
    ...
    for (i = 0; i < numOutputs; i++) {
      ...
      for (j = 0; j < numOutputs; j++) {
        xj = j % numCols;
        yj = j / numRows;
        distance[i * numOutputs + j] = (int) pow(((xi - xj) * (xi - xj)), 2.0) + (int) pow(((yi - yj) * (yi - yj)), 2.0);
      }
    }
  }
  ...
  void adjustNeighborhood() {
    ...
    learnRate = initLearnRate * pow(finalLearnRate / initLearnRate, ratio);
    sigma = (double) numCols * pow((0.20 / (double) numCols), ratio);
  ...
  void adjustWeights() {
    ...
    for (i = 0; i < numOutputs; i++) {
      dist = exp((distance[winner * numOutputs + i] * -1.0) / (2.0 * sigma_squared));
  ...
  void reset() {
    ...
    for (i = 0; i < weights.length; i++) {
      weights[i] = 0.6 - (0.2 * random());  // between 0.4 and 0.6
    }
  }
